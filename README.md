...
| DB / Vector | Cloud SQL for PostgreSQL（pgvector 拡張） |
| LLM | Vertex AI / Gemini API (Gemini 2.5 Flash) |
| Embedding | Vertex AI / Gemini API (gemini-embedding-001) |
| バッチ | ローカル Docker（Ingest Pipeline） |
| テスト | pytest |
| ホスティング | Cloud Run（フロントエンド＆バックエンド） |
...
```bash
cp .env.example .env
# .env を編集して DATABASE_URL・GEMINI_API_KEY などを設定
```
...
| 13 | [商用環境構築手順書](docs/13_商用環境構築手順書.md) | 環境構築 | 本番デプロイ（GCP / Terraform版） |
| 14 | [RAGデータ投入マニュアル](docs/14_RAGデータ投入マニュアル.md) | 運用 | データ取り込み手順・メタ情報設計・Proxy接続 |
| 15 | [運用マニュアル](docs/15_運用マニュアル.md) | 運用 | 監視・スケーリング設定・ログ確認 |
| 16 | [運用コスト見積もり](docs/16_運用コスト見積もり.md) | 運用 | GCP運用費用シミュレーション（節約・本番プラン） |
| 17 | [動画台本（NotebookLM用）](docs/17_動画台本（NotebookLM用）.md) | その他 | 動画用の台本スクリプト |
